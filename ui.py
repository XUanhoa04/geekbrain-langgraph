...
def get_relevant_snippet(chunk_text, answer):
    ...
    stopwords = {"the", "a", "an", "is", "are", "was", "were", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with", "by", "from", "this", "that", "it", "they", "we", "you", "i", "he", "she", "as", "be", "have", "has", "had"}
    # Lấy các từ khóa thực sự
    keywords = set(w for w in clean_ans.split() if w not in stopwords and len(w) > 1)
...
# ================== PAGE CONFIG ==================
st.set_page_config(
    page_title="GeekBrain AI Assistant",
    page_icon="🧠",
    layout="wide",
...
<div class="main-header">
    <h1>🧠 GeekBrain AI Assistant</h1>
    <p>Powered by Group 4</p>
</div>
""", unsafe_allow_html=True)
                ...
                for i, doc in enumerate(msg["sources"]):
                    ...
                    for idx, chunk in enumerate(chunks):
                        ...
                        if match:
                            ...
                            title = title_line if title_line.startswith("[Source:") else f"Chunk {idx+1}"
...
if prompt := st.chat_input("Ask about GeekBrain — teams, costs, metrics, policies..."):
    # Add user message
    ...
    with st.chat_message("assistant"):
        ...
        with st.status("🔄 **Quá trình Reasoning...** (Live)", expanded=True) as status:
                        ...
                        if isinstance(messages, list) and len(messages) > 0:
                            ...
                            if hasattr(last_msg, 'tool_calls') and last_msg.tool_calls:
                                if hasattr(last_msg, 'content') and last_msg.content:
                                    ...
                                    thought = last_msg.content.replace('<thinking>', '').replace('</thinking>', '').strip()
                                    if thought:
            ...
            status.update(label="✅ **Quá trình Reasoning hoàn tất**", state="complete", expanded=False)
        ...
        # Get final answer
            ...
            if thinking_match:
                ...
                if final_reasoning:
                    with st.expander("🤔 **Final Reasoning** (Giải thích lựa chọn dữ liệu)", expanded=True):
                        st.info(final_reasoning)
                    current_traces.append(f'<div class="trace-step">🤔 <b>Final Reasoning</b><br><i>{final_reasoning}</i></div>')
            ...
            tools_html = " ".join([f'<span style="background:#4CAF50; color:white; padding:2px 6px; border-radius:10px; font-size:10px; margin-right:4px;">{t}</span>' for t in sorted(list(used_tools))])
