...
SERVICE_NAME_RE = re.compile(
    r"\b([A-Za-z][A-Za-z0-9-]{1,62}(?:Svc|Service|GW|Gateway|Detector))\b",
    re.IGNORECASE,
)
...
def _service_terms(service: str) -> set[str]:
    ...
    words = re.sub(r"([a-z0-9])([A-Z])", r"\1 \2", service).replace("-", " ")
    ...
    for suffix, expanded in suffixes.items():
        if normalized.endswith(suffix):
            stem = normalized[: -len(suffix)]
            terms.update({stem + expanded, stem + suffix})
...
def match_services(text: str, catalog: Iterable[str] = ()) -> list[str]:
    ...
    for match in SERVICE_NAME_RE.finditer(text):
        ...
        if candidate.lower() in {"microservice", "service", "gateway"}:
            continue
